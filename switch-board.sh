...
#                Primary build-time Wi-Fi network, seeded into NVS on boot.
#   WIFI_ENV_FILE=<path>
#                dotenv with WIFI_SSID[_2..8]/WIFI_PASSWORD[_2..8]
#                (default: wifi.env when present). See wifi.env.example.
#
# Examples:
...
# Regenerate main/wifi_networks.local.h (gitignored) from build-time Wi-Fi
# inputs, so WifiBoard can seed known networks into NVS without a serial
# "!wifi" step and without committing credentials.
#
# Inputs (all optional; env vars win over the file for slot 1):
#   WIFI_SSID / WIFI_PASSWORD          — primary network
#   wifi.env (or $WIFI_ENV_FILE)       — dotenv-style KEY=VALUE, one per line:
#       WIFI_SSID[_2..8] / WIFI_PASSWORD[_2..8]
# The file is parsed, never executed. With no inputs the header defines an
# empty WIFI_NETWORKS_LOCAL and nothing is seeded.
generate_wifi_header() {
    local header="main/wifi_networks.local.h"
    local wifi_env="${WIFI_ENV_FILE:-wifi.env}"
    local tmp key val i any=0
    ...
    if [ -f "$wifi_env" ]; then
        while IFS='=' read -r key val; do
            ...
            val="${val%\'}"; val="${val#\'}"
            case "$key" in
                WIFI_SSID)            ssids[1]="$val" ;;
                WIFI_PASSWORD)        passes[1]="$val" ;;
                WIFI_SSID_[2-8])      ssids[${key#WIFI_SSID_}]="$val" ;;
                WIFI_PASSWORD_[2-8])  passes[${key#WIFI_PASSWORD_}]="$val" ;;
            esac
        done < "$wifi_env"
    fi
    [ -n "${WIFI_SSID:-}" ]     && ssids[1]="$WIFI_SSID"
    [ -n "${WIFI_PASSWORD:-}" ] && passes[1]="$WIFI_PASSWORD"

    tmp=$(mktemp)
    {
        echo "// Generated by switch-board.sh from wifi.env / WIFI_SSID env."
        echo "// DO NOT COMMIT, DO NOT EDIT — regenerated on every build."
        printf '#define WIFI_NETWORKS_LOCAL'
        for i in 1 2 3 4 5 6 7 8; do
            [ -n "${ssids[$i]:-}" ] || continue
            printf ' \\\n    {"%s", "%s"},' \
                "$(c_escape "${ssids[$i]}")" "$(c_escape "${passes[$i]:-}")"
            any=1
        done
    ...
    if [ ! -f "$header" ] || ! cmp -s "$tmp" "$header"; then
        mv "$tmp" "$header"
        if [ "$any" -eq 1 ]; then
            echo ">>> Wi-Fi: baked $(grep -o '{' "$header" | wc -l | tr -d ' ') network(s) into $header" >&2
        else
            echo ">>> Wi-Fi: no wifi.env / WIFI_SSID — $header has no networks" >&2
        fi
    else
        rm -f "$tmp"
    fi
